...
def printmap():
  print(f"\033[2K\n\033[2K{entitys[0].name}(プレイヤー)")
  print(f"\033[2KHP:{entitys[0].HP} 攻撃力:{entitys[0].AP}")
  if entitys[0].equipment!=-1:
    print(f"\033[2K装備品:{items[entitys[0].equipment].name}(追加攻撃力:{items[entitys[0].equipment].addAP})]")
  else:
    print("\033[2K装備品:なし")
  for y in range(mapraw[0][0]):
    ...
    if y!=mapraw[0][0]-1:
      print("")
  if flag>0:
    print(f"\033[{mapraw[0][0]+3}F",end="")
